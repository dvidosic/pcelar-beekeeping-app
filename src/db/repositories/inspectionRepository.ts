...
export async function insertInspection(inspection: Inspection): Promise<void> {
  ...
  await db.runAsync(
    `INSERT INTO inspections (
      id, hive_id, inspected_at, brood_quantity, brood_quality,
      queen_seen, queen_age, food_stores, temperament, hygienic_behavior,
      honey_intake_daily_g, health_status, treatment_applied, treatment_substance,
      treatment_date, swarm_event, swarm_destination_hive_id, notes, created_at
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      inspection.id, inspection.hive_id, inspection.inspected_at,
      inspection.brood_quantity, inspection.brood_quality,
      inspection.queen_seen, inspection.queen_age, inspection.food_stores,
      ...
      inspection.swarm_destination_hive_id, inspection.notes, inspection.created_at,
    ]
  );
}
...
export async function updateInspection(
  ...
  await db.runAsync(
    `UPDATE inspections SET
      ...
      queen_seen=?, queen_age=?, food_stores=?, temperament=?, hygienic_behavior=?,
      honey_intake_daily_g=?, health_status=?, treatment_applied=?, treatment_substance=?,
      treatment_date=?, swarm_event=?, swarm_destination_hive_id=?, notes=?
    WHERE id=?`,
    [
      data.inspected_at, data.brood_quantity, data.brood_quality,
      data.queen_seen, data.queen_age, data.food_stores,
      ...
      data.swarm_destination_hive_id, data.notes, id,
    ]
  );
}
